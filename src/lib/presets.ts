export interface Pattern {
  name: string;
  cells: [number, number][];
  gridSize?: number;
}

export const PRESETS: Pattern[] = [
  {
    name: "Gosper Glider Gun",
    gridSize: 50,
    cells: [
      [5, 1], [5, 2], [6, 1], [6, 2],
      [5, 11], [6, 11], [7, 11], [4, 12], [8, 12], [3, 13], [9, 13], [3, 14], [9, 14], [6, 15], [4, 16], [8, 16], [5, 17], [6, 17], [7, 17], [6, 18],
      [3, 21], [4, 21], [5, 21], [3, 22], [4, 22], [5, 22], [2, 23], [6, 23], [1, 25], [2, 25], [6, 25], [7, 25],
      [3, 35], [4, 35], [3, 36], [4, 36]
    ]
  },
  {
    name: "Pulsar",
    gridSize: 40,
    cells: [
      [2, 4], [2, 5], [2, 6], [2, 10], [2, 11], [2, 12],
      [4, 2], [5, 2], [6, 2], [4, 7], [5, 7], [6, 7], [4, 9], [5, 9], [6, 9], [4, 14], [5, 14], [6, 14],
      [7, 4], [7, 5], [7, 6], [7, 10], [7, 11], [7, 12],
      [9, 4], [9, 5], [9, 6], [9, 10], [9, 11], [9, 12],
      [10, 2], [11, 2], [12, 2], [10, 7], [11, 7], [12, 7], [10, 9], [11, 9], [12, 9], [10, 14], [11, 14], [12, 14],
      [14, 4], [14, 5], [14, 6], [14, 10], [14, 11], [14, 12]
    ]
  },
  {
    name: "Pentadecathlon",
    gridSize: 40,
    cells: [
      [7, 2], [7, 3], [6, 4], [8, 4], [7, 5], [7, 6], [7, 7], [7, 8], [6, 9], [8, 9], [7, 10], [7, 11]
    ]
  },
  {
    name: "Copperhead Spaceship",
    gridSize: 40,
    cells: [
      [2, 5], [2, 6], [3, 4], [3, 7], [4, 4], [4, 7], [5, 3], [5, 8], [6, 3], [6, 8], 
      [8, 4], [8, 7], [9, 4], [9, 7], [10, 5], [10, 6], [11, 5], [11, 6], [13, 5], [13, 6], [14, 4], [14, 7]
    ]
  },
  {
    name: "Glider Fleet",
    gridSize: 50,
    cells: [
      [0, 1], [1, 2], [2, 0], [2, 1], [2, 2],
      [5, 6], [6, 7], [7, 5], [7, 6], [7, 7],
      [10, 11], [11, 12], [12, 10], [12, 11], [12, 12],
      [15, 16], [16, 17], [17, 15], [17, 16], [17, 17],
      [20, 21], [21, 22], [22, 20], [22, 21], [22, 22]
    ]
  }
];
